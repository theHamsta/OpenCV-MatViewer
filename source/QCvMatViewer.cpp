...
QCvMatViewer::QCvMatViewer( QWidget* parent ):
	...
	QOpenGLFunctions(),
#ifdef NDEBUG
	m_backgroundColor{OGL_BLACK}
#else
	m_backgroundColor{OGL_PURPLE } // purple background for debugging (virtually no video has this color)
#endif
{
}
...
void QCvMatViewer::paintPoints()
{
	if ( !m_pointMat.empty() && m_drawPoints ) {
		...
		if ( m_numPointsPerColor == 0 ) {
			...
			assert( numImgs < sizeof( someGlColors ) / 3 / sizeof( float ) && "Define more colors in global array someGlColors" );
...
void QCvMatViewer::paintLines()
{
	if ( !m_lines.empty() && m_drawTriangles ) {
		...
		glColor4f( OGL_CYAN, 0.3f );
		glDrawArrays( GL_LINES, 0, m_lines.total() );
...
void QCvMatViewer::setLinesToDraw( const cv::Mat& pointPairs )
{
	m_lines = pointPairs;
...
void QCvMatViewer::loadMatIntoTex( cv::Mat& mat, GLuint tex )
{
	...
	switch ( mat.type() ) {
		...
		case CV_8UC3:
			if ( mat.step % 4 != 0/*(mat.cols * 3) % 4 != 0*/ ) { // Rows not alligned to int32
				qWarning() << "Removed" << ( 4 - ( mat.cols * 3 ) % 4 ) % 4
				           << "pixels columns on the right to display CV_8UC3 image correctly on GPU (and made an additional copy of the image)";
				mat = mat( cv::Rect( 0, 0, mat.cols - ( 4 - ( mat.cols * 3 ) % 4 ) % 4, mat.rows ) ).clone();
				glPixelStorei( GL_UNPACK_ROW_LENGTH, mat.step / mat.elemSize() );
				assert( ( mat.cols * 3 % 4 ) == 0 && "Rows must be alligned to word size (4 bytes) to be displayed correctly on GPU" );
			}
...
void QCvMatViewer::mouseMoveEvent( QMouseEvent* event )
{
	...
	if ( m_interactiveMode && m_drag ) {
		...
		m_glTranslateY += ( m_oldMousePos.y() - event->pos().y() ) / m_glZoom; // y-inverted because Qt and GL use different coordinate systems
		m_oldMousePos = event->pos();
		update();
	}
	...
	if ( m_interactiveMode && m_dragZoom ) {
		...
		m_glZoom += ( m_oldMousePos.y() - event->pos().y() ) * m_glZoom / 300.f; // y-inverted because Qt and GL use different coordinate systems
...
void QCvMatViewer::mouseReleaseEvent( QMouseEvent* event )
{
	...
	if ( m_subImgSelectionActivated ) {
		...
		// This widget shows a scaled version of the image. So the selection must be scaled back to the image. Also subtract the video offset relative to the widget before.
		cv::Rect subImgRect( ( selectionInVideo.x() - m_leftX ) / m_scale,
		                     ( selectionInVideo.y() - m_bottomY ) / m_scale,
		                     selectionInVideo.width() / m_scale,
		                     selectionInVideo.height() / m_scale );

		if ( subImgRect.width > 0 && subImgRect.height > 0 ) {
			qDebug() << "Sub-region" << QRect( subImgRect.x, subImgRect.y, subImgRect.width, subImgRect.height ) << "selected";
...
void QCvMatViewer::showHighlightedRect( QRect rect, HighlighCoordinates coordinateSystem, float colorR, float colorG, float colorB, float colorA )
{
	m_highlightRec = rect;
